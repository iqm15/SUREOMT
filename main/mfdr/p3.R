...
param.list <- fromJSON(file = "../../config/mfdr/p3.json")  
# -------------------------------------------------------------------------------
...
data_mfdr <- matrix(data = NA, nrow = nb_proc, ncol = length(p3), byrow = FALSE)  
...
for (i in 1:length(p3)) {    
  data <- compare_mfdr_proc(param.list$alpha, param.list$w0,
                            ...
                            param.list$non_nulls_proportion, p3[i],    
...
file_name_data_power = gsub(" " , "", paste("../../data/mfdr/p3/", gsub(" ", "_", paste("data_power", param.list$param_interest, "study", now(), sep="_")), ".csv"))
file_name_data_mfdr = gsub(" " , "", paste("../../data/mfdr/p3/", gsub(" ", "_", paste("data_mfdr", param.list$param_interest, "study", now(), sep="_")), ".csv"))
write.csv(df_power, file_name_data_power, row.names = FALSE)
write.csv(df_mfdr, file_name_data_mfdr, row.names = FALSE)

# ---------------------- make plor and save it ----------------------------------
