...
param.list <- fromJSON(file = "../../config/mfdr/lambda.json")  
# -------------------------------------------------------------------------------
...
data_mfdr <- matrix(data = NA, nrow = nb_proc, ncol = length(lambda), byrow = FALSE)  

# -------------------------------------------------------------------------------------

for (i in 1:length(lambda)) {    
  data <- compare_mfdr_proc(param.list$alpha, (1 - lambda[i]) * param.list$alpha,
                            param.list$nb_run,
...
file_name_data_power = gsub(" " , "", paste("../../data/mfdr/lambda/", gsub(" ", "_", paste("data_power", param.list$param_interest, "study", now(), sep="_")), ".csv"))
file_name_data_mfdr = gsub(" " , "", paste("../../data/mfdr/lambda/", gsub(" ", "_", paste("data_mfdr", param.list$param_interest, "study", now(), sep="_")), ".csv"))
...
data_power <- data_power %>% filter(procedure == "ALORD" | procedure == "SAFFRON" | procedure == "rhoALORD")
data_mfdr <- data_mfdr %>% filter(procedure == "ALORD" | procedure == "SAFFRON" | procedure == "rhoALORD")
